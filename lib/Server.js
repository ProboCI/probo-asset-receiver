...
var Server = function(options) {
  ...
  this.homeRedirect = this.options.homeRedirect || ''
...
Server.prototype.setup = function() {
  var homeRedirect = this.homeRedirect;
  var app = express();
  var loggerOptions = {
    logger: this.logger,
  };
  ...
  app.get('/', function(req, res) {
    if (homeRedirect) {
      ...
      res.send('ProboCI Asset Receiver.');
    }
  });
...
Server.prototype.routes.index = function(req, res, done) {
  ...
  if (homeRedirect) {
    ...
    res.end('ProboCI Asset Receiver.');
  }
};
  ...
  this.database.getBucketFromToken(token, function(error, bucket) {
    ...
    self.database.createAsset(bucket, assetName, assetId, metadata, function(error) {
      ...
      req
        ...
        .pipe(crypto.createCipheriv(self.options.encryptionCipher, (self.options.encryptionPassword + assetId + bucket).substr(0, 32), new Uint8Array(16)))
...
Server.prototype.routes.serveFileAsset = function(req, res, done) {
  ...
  self.database.getAssetId(bucket, assetName, function(error, assetId) {
    ...
    self.fileStorage.createReadStream(assetId)
      .pipe(crypto.createDecipheriv(self.options.encryptionCipher, (self.options.encryptionPassword + assetId + bucket).substr(0, 32), new Uint8Array(16)))
