...
Server.prototype.setup = function() {
  ...
  app.del('/buckets/:bucket/token/:token', this.routes.deleteBucketToken);
...
Server.prototype.routes.listBucketTokens = function(req, res, done) {
  readStream = this.database.listBucketTokens(req.params.bucket);
  res.writeHead(200, { 'Content-Type': 'application/JSON' });
  res.write('[\n');
  var first = true;
  readStream
...
Server.prototype.routes.createBucket = function(req, res, done) {
  ...
  self.database.createBucket(bucket, req.body, function(error) {
    if (error) return self.handleError(req, res, error, done);
    res
      .status(201)
      .send('Bucket created');
  });
...
Server.prototype.handleError = function(req, res, error, done) {
  var self = this;
